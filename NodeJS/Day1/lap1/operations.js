...
function writeDataAsync() {
    fs.promises.writeFile("students.json", JSON.stringify(studentData, null, 2))
        .then(() => {
...
function readDataAsync() {
    return fs.promises.readFile("students.json", "utf-8")
        .then(data => {
...
function addStudentAsync(newStudent) {
    readDataAsync()
        .then(data => {
            data.push(newStudent);
            return fs.promises.writeFile("students.json", JSON.stringify(data, null, 2));
        })
        .then(() => {
...
// updateStudentCourseSync(2, "Artificial Intelligence");
// updateStudentCourseAsync(3, "Full Stack Development");
...
function deleteStudentAsync(studentId) {
    readDataAsync()
        .then(data => {
            const filteredData = data.filter(s => s.id !== studentId);
            return fs.promises.writeFile("students.json", JSON.stringify(filteredData, null, 2));
        })
        .then(() => {
